...
with open(sys.argv[1], "r") as text_file, open(sys.argv[2], "wb") as outfile, open(sys.argv[3], "w+") as outfile_2:
	data = pd.read_csv(sys.argv[1],header=0,sep=',',error_bad_lines=False,engine='python')
...
#data_pivot_table ==> Also sets max value to 100. 
	data = data.pivot_table(index =['Species','Type','Plasmid','Gene','Length'],columns=['Species_Strain','ST','Strain_name'],values=['%ID'],aggfunc='sum',fill_value=0)
	data = data.clip_upper(100)
